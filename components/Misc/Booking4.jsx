...
const Booking4 = ({ loader, setLoader }) => {
  const { reduxData, handleLocalData, confirmation } =
  ...
  const calculatePrice = async () => {
    try {
      ...
      if (reduxData?.appData?.user?.id) {
        // customer coupons
        const customerCouponsResponse = await axios.post(
          "https://takemihome.it/it/front/booking/customerCoupons",
          {
        ...
        // corporate coupons
        const corporateCouponsResponse = await axios.post(
          "https://takemihome.it/it/front/booking/corporateCoupons",
          {
            customer: reduxData?.appData?.user?.id,
          }
        );
  ...
  const submitCoupon = () => {
    ...
    axios
      .post("https://takemihome.it/en/submit_gift", data)
      .then((res) => {
        if (res.data.success == true) {
          ...
          alert(
            `Gift card applied applied, Amount reduced is: ${calculatedAmount}`
          );
        } else {
  ...
  const handleAddress = async (data) => {
    ...
    const res = await SetNewPrimaryAddress(
      addressData?.address1,
      addressData?.address2,
      addressData?.city,
      addressData?.id,
  ...
  const discountGroup = () => {
    if (reduxData?.appData?.user?.id) {
      axios
        .post("https://takemihome.it/it/front/booking/customerCoupons", {
          customer: reduxData?.appData?.user?.id,
        })
        .then((res) => {
  ...
  const corporateDiscountGroupData = () => {
    if (reduxData?.appData?.user?.id) {
      axios
        .post("https://takemihome.it/it/front/booking/corporateCoupons", {
          customer: reduxData?.appData?.user?.id,
        })
        .then((res) => {
          if (res?.data?.coupons.length > 0) {
        ...
        <div className="w-full border p-5 ">
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="flex justify-between items-center">
              ...
              <p className="font-bold">{reduxData?.appData?.totalAmount} €</p>
            </div>

            {!priceTableLoader ? (
              <>
                {priceTableData?.percentageAmount > 0 && (
                  <div className="flex justify-between items-center">
                    <p className="mt-3">Amount after Outside Extra Fee is: </p>
                    <pre> </pre>
                    <p className="font-bold">
                      {priceTableData?.percentageAmount} €
                    </p>
                  </div>
                )}

                {priceTableData?.customerCoupons > 0 && (
                  <div className="flex justify-between items-center">
                    <p className="mt-3">Customer Coupon Amount is: </p>
                    <pre> </pre>
                    <p className="font-bold">
                      {priceTableData?.customerCoupons} €
                    </p>
                  </div>
                )}

                {priceTableData?.corporateCoupons > 0 && (
                  <div className="flex justify-between items-center">
                    <p className="mt-3">Corporate Coupon Amount is: </p>
                    <pre> </pre>
                    <p className="font-bold">
                      {priceTableData?.corporateCoupons} €
                    </p>
                  </div>
                )}

                {priceTableData?.giftAmount > 0 && (
                  <div className="flex justify-between items-center">
                ...
                {amountAfterCalculations && (
                  <div className="flex justify-between items-center">
                    ...
                    <p className="font-bold">{amountAfterCalculations} €</p>
                  </div>
                )}
              </>
            ) : (
            ...
            <div className="iconBox flex">
              <p className="w-full">
                <div className="flex w-full mb-2">
                  ...
                  <select
                    className="border w-full address rounded"
                    onChange={(e) =>
                      handleAddressChange(JSON.parse(e.target.value))
                    }
                  >
              ...
              <div className="iconBox flex">
                <p className="w-full">
                  <div className="flex w-full">
                    <label className="w-2/3 flex items-center">
                      Enter Address 1
                    </label>
                    <input
                      className="border w-full rounded p-1 pl-2"
                  ...
                  <div className="flex w-full">
                    ...
                    <input
                      className="border w-full p-1 pl-2 rounded"
                ...
                <div className="iconBox flex">
                  <p className="w-full">
                    <div className="flex w-full">
                      ...
                      <input
                        className="border w-full address"
                        onChange={(e) => {
                          setAddressData({
                            ...addressData,
                    ...
                    {({
                      ...
                      <form onSubmit={handleSubmit}>
                        ...
                        <button
                          ...
                          className="px-3 py-2  text-[1rem] rounded button-filled yellow-button w-full overflow-hidden"
                        >
                          Submit
                        </button>
                        <button
                          onClick={(e) => resetPercentage(e)}
                          className="w-full rounded flex gap-2 justify-center items-center focus:outline-none  bg-black hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mt-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                        >
                          Reset
                        </button>
                      </form>
              ...
              <div className="w-12/12 ">
                <div className="flex items-left flex-col">
                  ...
                  <input
                    ...
                    className="mb-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700"
                  />
                  <button
                    onClick={() => {
                      submitCoupon();
                    }}
                    class="mb-2 mt-1 px-3 py-2 rounded button-filled yellow-button w-full overflow-hidden"
                  >
                    Submit
                  </button>
                  <button
                    ...
                    class="w-full rounded flex gap-2 justify-center items-center focus:outline-none  bg-black hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                  >
                    Reset
                  </button>
                </div>
          ...
          {groupDiscount && (
            <div>
              ...
              <div className="flex justify-between">
                <p>The discount from {groupDiscount?.name} is:</p>
                <p className="font-bold">{groupDiscount?.discount} %</p>
              </div>
            </div>
          )}
            ...
            <div class="flex p-5 rounded-lg shadow bg-white justify-center">
              <div class="ml-3 text-center flex flex-col justify-center gap-3">
                ...
                <p class="mt-2 text-sm text-gray-600 leading-relaxed">
                  Booking has been created Successfully. Please Check Your
                  Email.
                </p>
                <button
                  type="button"
