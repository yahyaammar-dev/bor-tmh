...
const loginUser = async (values) => {
  try {
    const res = await axios.post(
      "http://127.0.0.1:8000/it/api/public_api/subcategories/loginAdminfpa",
      {
        email: values.email,
...
const intiateBooking = async (day, time, duration, city, service, pro, customer) => {
  try {
    const res = await axios.get(
      `http://127.0.0.1:8000/it/front/booking/set_new_appointment?day=04-08-2023&time=09:30&duration=120&city=1&service=[1200,17]&pro=16&customer=26`
    );
    return res.data;
  } catch (err) {
    alert("Unable to fetch corporate professionals, something went wrong"); // Throw a custom error message
